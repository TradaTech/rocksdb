...
make('iterator optimized for seek', function (db, t, done) {
  ...
  var ite = db.iterator()
  ite.next()
  t.equal(ite.value()[0].toString(), 'a', 'key matches')
  t.equal(ite.cache.length, 0, 'no cache')
  ite.next()
  t.equal(ite.value()[0].toString(), 'b', 'key matches')
  t.ok(ite.cache.length > 0, 'has cached items')
  ite.seek('d')
  t.notOk(ite.cache, 'cache is removed')
  ite.next()
  t.equal(ite.value()[0].toString(), 'd', 'key matches')
  t.equal(ite.cache.length, 0, 'no cache')
  ite.next()
  t.equal(ite.value()[0].toString(), 'e', 'key matches')
  t.ok(ite.cache.length > 0, 'has cached items')
  ite.end()
  done()
})
